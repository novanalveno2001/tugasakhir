...
import 'package:tugasakhir/data_categories/datakeluarga.dart';
...
class Keluarga extends StatefulWidget {
  const Keluarga({Key? key}) : super(key: key);
...
class _KeluargaState extends State<Keluarga> {
  ...
  Widget build(BuildContext context) {
    ...
    createTile(Users keluarga) => Hero
      (
      tag: keluarga.title,
      child: Material(
        elevation: 16.0,
          ...
          child: Stack(
            children: <Widget>[
              Column(
                ...
                children: [
                  Expanded(child: Image(image: NetworkImage(keluarga.image))),
                  SizedBox(height: 10),
                  Container(
                    child: Text(
                      keluarga.title,
      ...
      slivers: <Widget>[
        SliverPadding(
          ...
          sliver: SliverGrid.count(
            ...
            children: listUsers.map((keluarga) => createTile(keluarga)).toList(),
          ),
        )
      ],
    );
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('Kategori Parenting Family'),
        actions: [
          IconButton(
              onPressed: (){
